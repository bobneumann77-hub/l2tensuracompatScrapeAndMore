...
public class GatewayConfigGen extends PlaceboCodecDataGen<Gateway> {
   ...
   private static HostilityGateways slimeGate() {
      ...
      return new HostilityGateways(
         (MobTrait)LHTraits.GROWTH.get(),
         2,
         2000,
         Items.SLIME_BALL,
         List.of(
            wave(100, 1200, slime(EntityType.SLIME, 2, 4)),
            wave(100, 1800, slime(EntityType.SLIME, 2, 3), slime(EntityType.MAGMA_CUBE, 2, 1)),
            wave(120, 2400, slime(EntityType.SLIME, 2, 2), slime(EntityType.MAGMA_CUBE, 2, 2)),
            wave(
               120, 3000, slime(EntityType.SLIME, 3, 1), slime(EntityType.MAGMA_CUBE, 3, 1), slime(EntityType.SLIME, 2, 2), slime(EntityType.MAGMA_CUBE, 2, 2)
            ),
            wave(140, 3600, slime(EntityType.SLIME, 3, 3), slime(EntityType.MAGMA_CUBE, 3, 3))
         ),
         List.of(
   ...
   private static HostilityGateways splitGate() {
      EntityType[] types = new EntityType[]{
         EntityType.ZOMBIE, EntityType.DROWNED, EntityType.HUSK, EntityType.SKELETON, EntityType.WITHER_SKELETON, EntityType.STRAY
      };
      ...
      return new HostilityGateways(
         (MobTrait)LHTraits.SPLIT.get(),
         2,
         2000,
         Items.ROTTEN_FLESH,
         List.of(
            wave(100, 1200, entity(EntityType.ZOMBIE, 4)),
            wave(100, 1800, entity(EntityType.SKELETON, 2), entity(EntityType.ZOMBIE, 2)),
            wave(120, 2400, entity(EntityType.DROWNED, 2), entity(EntityType.SKELETON, 2), entity(EntityType.HUSK, 2)),
            wave(
               120,
               3000,
   ...
   public static void genConfig(Collector collector) {
      for (HostilityGateways e : GATEWAYS) {
         ...
         collector.add(L2Hostility.ENTITY, ResourceLocation.fromNamespaceAndPath("gateways", e.trait().getRegistryName().getPath()), config);
      }
   }

   public static void genRecipe(RegistrateRecipeProvider pvd) {
      for (HostilityGateways e : GATEWAYS) {
         ItemStack result = ((Item)GatewayObjects.GATE_PEARL.value()).getDefaultInstance();
         GatePearlItem.setGate(result, GatewayRegistry.INSTANCE.holder(e.path()));
         RecipeGen.<ShapedRecipeBuilder>unlock(pvd, ShapedRecipeBuilder.shaped(RecipeCategory.MISC, result)::unlockedBy, (Item)LCItems.VOID_EYE.get())
            .pattern("1A1")
            .pattern("ABA")
            .pattern("1A1")
            ...
            .define('1', e.mat())
            .save(ConditionalRecipeWrapper.mod(pvd, new String[]{"gateways"}), e.path());
      }
   }
   ...
   private void add(BiConsumer<String, Gateway> map, HostilityGateways gate) {
      map.accept(
         "gateways/gateways/hostility/" + gate.trait().getRegistryName().getPath(),
         new NormalGateway(
            Size.MEDIUM,
            TextColor.fromRgb(gate.trait().getColor()),
            gate.waves(),
            List.of(new StackReward(new ItemStack(gate.trait().asItem(), gate.count())), new XpReward(gate.xp(), gate.xp() / 10)),
            List.of(),
            SpawnAlgorithms.OPEN_FIELD,
            GateRules.DEFAULT,
            BossEventSettings.DEFAULT
         )
      );
   }
   ...
   private static WaveEntity slime(EntityType<?> type, int size, int count) {
      return new StandardWaveEntity(
         type, Optional.empty(), Optional.of((CompoundTag)Util.make(new CompoundTag(), e -> e.putInt("Size", (1 << size) - 1))), List.of(), false, count
      );
   }
   ...
   private static EntityConfig.Config config(ResourceLocation id, int start, int end, int count, int min, int base, EntityType<?>... types) {
      return EntityConfig.entity(min, base, 0.0, 0.0, List.of(types))
         .conditions(GatewayCondition.of(id, start, end, count, 1.0))
         .blacklist((MobTrait)LHTraits.MOONWALK.get(), (MobTrait)LHTraits.GRAVITY.get(), (MobTrait)LHTraits.INVISIBLE.get());
   }
}
