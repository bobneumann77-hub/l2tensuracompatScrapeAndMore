...
public enum LangData {
   ...
   TOOLTIP_WITCH_CHARGE(
      "tooltip.witch_charge",
      "Right click to fire. When hit entity, increase duration of all harmful effects longer than %ss by %s%%. At most increase to %ss.",
      3
   ),
   ...
   ITEM_WAND_ADDER(
      "item.wand.adder", "Right click blocks to select trait. Right click mobs to select trait rank. Press shift to select in opposite direction.", 0
   ),
   ...
   ITEM_BOOK_EVERYTHING_SHIFT(
      "item.book.everything_shift", "Shift right click with it to turn it into a book with all enchantments obtainable from enchantment table.", 0
   ),
   ITEM_BOOK_EVERYTHING_USE(
      "item.book.everything_use",
      "Rename this item with ID of an enchantment obtainable from enchantment table, then right click to generate a book of that enchantment, costing player experience.",
      0
   ),
   ...
   ITEM_RING_REFLECTION(
      "item.equipment.ring_of_reflection",
      "When a mob trait tries to apply a negative effect on you, apply it to surrounding enemies that target you instead.",
      0
   ),
   ITEM_FLAME_THORN(
      "item.equipment.flame_thorn", "When you damage a mob, inflict Soul Flame with level equals to total number of effects that mob has, for %ss", 1
   ),
   ...
   ABRAHADABRA(
      "item.equipment.abrahadabra", "When a mob trait tries to apply a trait effect on you, apply it to surrounding enemies that target you instead.", 0
   ),
   ...
   ABYSSAL_THORN(
      "item.equipment.abyssal_thorn",
      "Mobs get all possible traits at their level, but with Curse of Envy, they will always drop trait symbols of those when killed",
      0
   ),
   ...
   public static String perc(double val) {
      double perc = val * 100.0;
      int digit = (int)Math.round(perc);
      if (Math.abs(digit - perc) < 0.001) {
         return digit + "";
      }

      double thrc = val * 1000.0;
      int thgt = (int)Math.round(thrc);
      return Math.abs(thgt - thrc) < 0.001 ? thgt * 0.1 + "" : perc + "";
   }
}
