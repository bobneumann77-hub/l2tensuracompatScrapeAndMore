...
import dev.xkmc.l2complements.init.registrate.LCEnchantments.Order;
...
import dev.xkmc.l2core.init.reg.ench.EnchVal.Legacy;
...
public class LHEnchantments {
   ...
   public static final Legacy<RemoveTraitEnchantment> SPLIT_SUPPRESS;
   ...
   static {
      ...
      INSULATOR = DC.ench(
         ...
         e -> e.maxLevel(3).group(EquipmentSlotGroup.ARMOR).items(ItemTags.ARMOR_ENCHANTABLE).color(green).special(LCEnchantments.CRAFT, order.of(armor))
      );
      SPLIT_SUPPRESS = DC.enchLegacy(
         "split_suppressor",
         "Split Suppressor",
         "Disable Split trait on enemies on hit",
         e -> e.group(EquipmentSlotGroup.MAINHAND).items(ItemTags.WEAPON_ENCHANTABLE).color(green).special(LCEnchantments.CRAFT, order.of(weapon)),
