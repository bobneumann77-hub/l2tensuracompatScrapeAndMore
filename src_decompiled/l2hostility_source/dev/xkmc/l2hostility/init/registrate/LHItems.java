...
public class LHItems {
   public static final ItemEntry<HostilityOrb> HOSTILITY_ORB = L2Hostility.REGISTRATE
      .item("hostility_orb", p -> new HostilityOrb(p.stacksTo(64).rarity(Rarity.EPIC)))
      .register();
   public static final ItemEntry<BottleOfCurse> BOTTLE_CURSE = L2Hostility.REGISTRATE
      .item("bottle_of_curse", p -> new BottleOfCurse(p.stacksTo(64).rarity(Rarity.RARE).craftRemainder(Items.GLASS_BOTTLE)))
      .register();
   public static final ItemEntry<BottleOfSanity> BOTTLE_SANITY = L2Hostility.REGISTRATE
      .item("bottle_of_sanity", p -> new BottleOfSanity(p.stacksTo(64).rarity(Rarity.RARE).craftRemainder(Items.GLASS_BOTTLE)))
      .register();
   public static final ItemEntry<Item> WITCH_DROPLET = L2Hostility.REGISTRATE.item("witch_droplet", Item::new).register();
   public static final ItemEntry<EffectBoosterBottle> BOOSTER_POTION = L2Hostility.REGISTRATE
      .item("booster_potion", p -> new EffectBoosterBottle(p.stacksTo(16).rarity(Rarity.RARE).craftRemainder(Items.GLASS_BOTTLE)))
      .register();
   public static final ItemEntry<HostilityChargeItem> WITCH_CHARGE = L2Hostility.REGISTRATE
      .item(
         "witch_charge",
         p -> new HostilityChargeItem(
            p,
            ChargeType.BOOST,
            () -> LangData.TOOLTIP_WITCH_CHARGE
               .get(
      ...
      .register();
   public static final ItemEntry<HostilityChargeItem> ETERNAL_WITCH_CHARGE = L2Hostility.REGISTRATE
      .item(
         "eternal_witch_charge",
         p -> new HostilityChargeItem(
            p,
            ChargeType.ETERNAL,
            () -> LangData.TOOLTIP_WITCH_ETERNAL.get((Integer)LHConfig.SERVER.witchChargeMinDuration.get() / 20).withStyle(ChatFormatting.GRAY)
         )
      )
      .register();
   ...
   public static final ItemEntry<Item> CHAOS_INGOT = L2Hostility.REGISTRATE
      .item("chaos_ingot", p -> new Item(p.rarity(Rarity.EPIC).fireResistant()))
      .tag(new TagKey[]{LHTagGen.BEACON_PAYMENT})
      .register();
   public static final ItemEntry<Item> HOSTILITY_ESSENCE = L2Hostility.REGISTRATE
      .item("hostility_essence", p -> new Item(p.rarity(Rarity.EPIC).fireResistant().component(DataComponents.ENCHANTMENT_GLINT_OVERRIDE, true)))
      .register();
   public static final ItemEntry<Item> MIRACLE_POWDER = L2Hostility.REGISTRATE
      .item("miracle_powder", p -> new Item(p.rarity(Rarity.EPIC).fireResistant().component(DataComponents.ENCHANTMENT_GLINT_OVERRIDE, true)))
      .register();
   public static final ItemEntry<Item> MIRACLE_INGOT = L2Hostility.REGISTRATE
      .item("miracle_ingot", p -> new Item(p.rarity(Rarity.EPIC).fireResistant().component(DataComponents.ENCHANTMENT_GLINT_OVERRIDE, true)))
      .tag(new TagKey[]{LHTagGen.BEACON_PAYMENT})
      .register();
   ...
   public static final ItemEntry<TraitAdderWand> ADDER = ((ItemBuilder)L2Hostility.REGISTRATE
         .item("trait_adder_wand", p -> new TraitAdderWand(p.stacksTo(1)))
         .model((ctx, pvd) -> pvd.handheld(ctx))
         .onRegister(x -> L2Hostility.REGISTRATE.modifyCreativeModeTab(LHBlocks.TAB.key(), e -> {
            e.accept(LMItems.WAND_AI.asStack());
            e.accept(LMItems.WAND_EQUIPMENT.asStack());
            e.accept(LMItems.WAND_TARGET.asStack());
         })))
      .register();
   public static final ItemEntry<SealedItem> SEAL = L2Hostility.REGISTRATE
      .item("sealed_item", p -> new SealedItem(p.stacksTo(1).fireResistant()))
      .removeTab(LHBlocks.TAB.key())
      .tag(new TagKey[]{LHTagGen.NO_SEAL})
      .register();
   ...
   private static <T extends Item> ItemBuilder<T, ?> curio(String str, NonNullFunction<Properties, T> factory) {
      return L2Hostility.REGISTRATE
         .item(str, factory)
         .model((ctx, pvd) -> pvd.generated(ctx, new ResourceLocation[]{pvd.modLoc("item/curio/" + ctx.getName())}));
   }
   ...
   static {
      ...
      WITCH_WAND = L2Hostility.REGISTRATE
         .item("witch_wand", p -> new WitchWand(p.durability(300).rarity(Rarity.EPIC).fireResistant()))
         .model((ctx, pvd) -> pvd.handheld(ctx))
         .tag(new TagKey[]{chaos})
         .register();
      ...
      LOOT_1 = curio("charm_of_looting_1", LootingCharm::new).tag(new TagKey[]{charm, pandora, LHTagGen.NO_SEAL}).lang("Unpolished Looting Charm").register();
      LOOT_2 = curio("charm_of_looting_2", LootingCharm::new).tag(new TagKey[]{charm, pandora, LHTagGen.NO_SEAL}).lang("Magical Looting Charm").register();
      LOOT_3 = curio("charm_of_looting_3", LootingCharm::new).tag(new TagKey[]{charm, pandora, LHTagGen.NO_SEAL}).lang("Chaotic Looting Charm").register();
      LOOT_4 = curio("charm_of_looting_4", LootingCharm::new).tag(new TagKey[]{charm, pandora, LHTagGen.NO_SEAL}).lang("Miraculous Looting Charm").register();
      ...
      ABYSSAL_THORN = curio("abyssal_thorn", AbyssalThorn::new).tag(new TagKey[]{chaos, curse}).lang("Abyssal Throne").register();
      ...
      RESTORATION = L2Hostility.REGISTRATE
         .item("pocket_of_restoration", p -> new PocketOfRestoration(p, 128))
         .model(
            (ctx, pvd) -> pvd.generated(ctx, new ResourceLocation[]{pvd.modLoc("item/curio/" + ctx.getName())})
               .override()
               .predicate(L2Hostility.loc("filled"), 0.5F)
               .model(
                  ((ItemModelBuilder)((ItemModelBuilder)pvd.getBuilder(ctx.getName() + "_full")).parent(new UncheckedModelFile("item/generated")))
                     .texture("layer0", "item/curio/" + ctx.getName() + "_full")
               )
         )
         .tag(new TagKey[]{charm, LHTagGen.NO_SEAL, ItemTags.DURABILITY_ENCHANTABLE})
         .register();
   }
}
