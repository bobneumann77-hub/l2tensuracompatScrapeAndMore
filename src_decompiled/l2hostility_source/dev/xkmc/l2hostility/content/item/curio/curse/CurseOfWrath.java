...
public class CurseOfWrath extends CurseCurioItem {
   public static final Set<Holder<MobEffect>> SET = new LinkedHashSet<>(
      List.of(MobEffects.BLINDNESS, MobEffects.DARKNESS, MobEffects.CONFUSION, MobEffects.MOVEMENT_SLOWDOWN, MobEffects.DIG_SLOWDOWN, MobEffects.WEAKNESS)
   );
   ...
   public int getExtraLevel() {
      return (Integer)LHConfig.SERVER.wrathExtraLevel.get();
   }

   public void appendHoverText(ItemStack stack, TooltipContext ctx, List<Component> list, TooltipFlag flag) {
      String rate = LangData.perc((Double)LHConfig.SERVER.wrathDamageBonus.get());
      list.add(LangData.ITEM_CHARM_WRATH.get(rate).withStyle(ChatFormatting.GOLD));
      if (ctx.level() != null) {
         this.addTooltip(list, SET);
      }
   }
   ...
   public void onHurtTarget(ItemStack stack, LivingEntity user, Offence cache) {
      ...
      if (level > 0) {
         float rate = ((Double)LHConfig.SERVER.wrathDamageBonus.get()).floatValue();
   ...
   private void addTooltip(List<Component> list, Set<Holder<MobEffect>> set) {
      ...
      for (Holder<MobEffect> e : set) {
         map.put(e.getKey().location(), (MobEffect)e.value());
      }
