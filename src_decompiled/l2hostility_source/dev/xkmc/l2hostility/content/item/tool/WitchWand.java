...
public class WitchWand extends Item {
   private static MobEffectInstance getRandom(RegistryAccess access, int maxRank, RandomSource source) {
      ...
      return new EffectBuilder(ans).setDuration(ans.getDuration() * (Integer)LHConfig.SERVER.witchWandFactor.get()).ins;
   }
   ...
   public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
      ...
      if (!level.isClientSide) {
         ...
         int maxRank = ((PlayerDifficulty)((PlayerCapabilityHolder)LHMiscs.PLAYER.type()).getOrCreate(player)).maxRankKilled;
         MobEffectInstance ins = getRandom(player.registryAccess(), maxRank, player.getRandom());
         potion.set(
            DataComponents.POTION_CONTENTS, new PotionContents(Optional.empty(), Optional.of(((MobEffect)ins.getEffect().value()).getColor()), List.of(ins))
         );
