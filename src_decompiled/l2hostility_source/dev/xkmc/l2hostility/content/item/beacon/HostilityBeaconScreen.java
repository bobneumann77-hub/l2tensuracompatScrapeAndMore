...
public class HostilityBeaconScreen extends AbstractContainerScreen<HostilityBeaconMenu> implements ContainerListener {
      ...
      for (int i = 0; i < 3; i++) {
         ...
         for (int j = 0; j < n; j++) {
            HostilityBeaconScreen.BeaconPowerButton btn = new HostilityBeaconScreen.BeaconPowerButton(
               this.leftPos + 76 + j * 24 - w / 2, this.topPos + 22 + i * 25, i * 2 + j, i
            );
            btn.active = false;
   ...
   void updateButtons() {
      int i = ((HostilityBeaconMenu)this.menu).getLevels();
   ...
   protected boolean click(int btn) {
      ...
      if (player == null) {
         return false;
      } else if (((HostilityBeaconMenu)this.menu).clickMenuButton(player, btn) && Minecraft.getInstance().gameMode != null) {
         Minecraft.getInstance().gameMode.handleInventoryButtonClick(((HostilityBeaconMenu)this.menu).containerId, btn);
         return true;
      } else {
         return false;
      }
   ...
   class BeaconPowerButton extends HostilityBeaconScreen.BeaconScreenButton {
      ...
      protected void setEffect(int eff) {
         ...
         this.setTooltip(Tooltip.create(this.createEffectDescription((MobEffect)this.getEffect().value()), null));
      }
      ...
      protected MutableComponent createNarrationMessage() {
         return this.createEffectDescription((MobEffect)this.getEffect().value());
      }
   }
