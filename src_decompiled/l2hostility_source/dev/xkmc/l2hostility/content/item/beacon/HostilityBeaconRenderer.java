...
public class HostilityBeaconRenderer implements BlockEntityRenderer<HostilityBeaconBlockEntity> {
   ...
   public HostilityBeaconRenderer(Context ctx) {
   }
   ...
   public void render(HostilityBeaconBlockEntity be, float pTick, PoseStack pose, MultiBufferSource buffer, int light, int overlay) {
      ...
      for (int i = 0; i < list.size(); i++) {
         HostilityBeaconBlockEntity.Section sec = list.get(i);
         renderBeaconBeam(pose, buffer, pTick, time, y, i == list.size() - 1 ? 1024 : sec.getHeight(), sec.getColor());
         y += sec.getHeight();
      }
   }

   private static void renderBeaconBeam(PoseStack pose, MultiBufferSource buffer, float pt, long t, int y, int h, int col) {
      renderBeaconBeam(pose, buffer, BEAM_LOCATION, pt, 1.0F, t, y, h, col, 0.2F, 0.25F);
   }

   public static void renderBeaconBeam(
      PoseStack pose, MultiBufferSource buffer, ResourceLocation rl, float pt, float h0, long time, int y0, int h, int col, float r0, float r1
   ) {
      int y1 = y0 + h;
      pose.pushPose();
      pose.translate(0.5, 0.0, 0.5);
      float t0 = Math.floorMod(time, 40) + pt;
      float t1 = h < 0 ? t0 : -t0;
      float t2 = Mth.frac(t1 * 0.2F - Mth.floor(t1 * 0.1F));
      pose.pushPose();
      pose.mulPose(Axis.YP.rotationDegrees(t0 * 2.25F - 45.0F));
      float f12 = -1.0F + t2;
      float f13 = h * h0 * (0.5F / r0) + f12;
      renderPart(pose, buffer.getBuffer(RenderType.beaconBeam(rl, false)), col, y0, y1, 0.0F, r0, r0, 0.0F, -r0, 0.0F, 0.0F, -r0, 0.0F, 1.0F, f13, f12);
      pose.popPose();
      f12 = -1.0F + t2;
      f13 = h * h0 + f12;
      renderPart(
         pose, buffer.getBuffer(RenderType.beaconBeam(rl, true)), ARGB32.color(32, col), y0, y1, -r1, -r1, r1, -r1, -r1, r1, r1, r1, 0.0F, 1.0F, f13, f12
      );
      pose.popPose();
   }
   ...
   private static void renderPart(
      ...
      Pose posestack$pose = p_112156_.last();
      renderQuad(
   ...
   private static void addVertex(
      Pose p_324495_, VertexConsumer p_253894_, int p_254357_, int p_350652_, float p_253871_, float p_253841_, float p_254568_, float p_254361_
   ) {
      p_253894_.addVertex(p_324495_, p_253871_, p_350652_, p_253841_)
         .setColor(p_254357_)
   ...
   public AABB getRenderBoundingBox(HostilityBeaconBlockEntity blockEntity) {
      BlockPos pos = blockEntity.getBlockPos();
      return new AABB(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1.0, 1024.0, pos.getZ() + 1.0);
   }
}
