...
public class LHMobCommands extends HostilityCommands {
   ...
   protected static LiteralArgumentBuilder<CommandSourceStack> build() {
      return (LiteralArgumentBuilder<CommandSourceStack>)literal("mobs")
         .then(((RequiredArgumentBuilder)argument("targets", EntityArgument.entities()).then(level())).then(trait()));
   }

   private static LiteralArgumentBuilder<CommandSourceStack> trait() {
      return (LiteralArgumentBuilder<CommandSourceStack>)((LiteralArgumentBuilder)((LiteralArgumentBuilder)literal("trait")
               .then(((LiteralArgumentBuilder)literal("clear").requires(e -> e.hasPermission(2))).executes(mobRun(LHMobCommands::commandClearTrait))))
            .then(
               ((LiteralArgumentBuilder)literal("remove").requires(e -> e.hasPermission(2)))
                  .then(argument("trait", ResourceKeyArgument.key(LHTraits.TRAITS.key())).executes(mobTrait(LHMobCommands::commandRemoveTrait)))
            ))
         .then(
            ((LiteralArgumentBuilder)literal("set").requires(e -> e.hasPermission(2)))
               .then(
                  argument("trait", ResourceKeyArgument.key(LHTraits.TRAITS.key()))
                     .then(argument("rank", IntegerArgumentType.integer(0)).executes(mobTraitRank(LHMobCommands::commandSetTrait)))
               )
         );
   }

   private static LiteralArgumentBuilder<CommandSourceStack> level() {
      return (LiteralArgumentBuilder<CommandSourceStack>)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)literal(
                           "level"
                        )
                        .then(
                           ((LiteralArgumentBuilder)literal("rerollTrait").requires(e -> e.hasPermission(2)))
                              .executes(mobRun((mob, cap) -> cap.reinit(mob, cap.getLevel(), false)))
                        ))
                     .then(
                        ((LiteralArgumentBuilder)literal("rerollTraitNoSuppression").requires(e -> e.hasPermission(2)))
                           .executes(mobRun((mob, cap) -> cap.reinit(mob, cap.getLevel(), true)))
                     ))
                  .then(
                     ((LiteralArgumentBuilder)literal("setAndRerollTrait").requires(e -> e.hasPermission(2)))
                        .then(argument("level", IntegerArgumentType.integer(0)).executes(mobLevel((mob, cap, level) -> cap.reinit(mob, level, false))))
                  ))
               .then(
                  ((LiteralArgumentBuilder)literal("addAndRerollTrait").requires(e -> e.hasPermission(2)))
                     .then(
                        argument("level", IntegerArgumentType.integer(0))
                           .executes(mobLevel((mob, cap, level) -> cap.reinit(mob, cap.getLevel() + level, false)))
                     )
               ))
            .then(
               ((LiteralArgumentBuilder)literal("set").requires(e -> e.hasPermission(2)))
                  .then(argument("level", IntegerArgumentType.integer(0)).executes(mobLevel((mob, cap, level) -> commandSetLevel(cap, mob, level))))
            ))
         .then(
            ((LiteralArgumentBuilder)literal("add").requires(e -> e.hasPermission(2)))
               .then(
                  argument("level", IntegerArgumentType.integer(0)).executes(mobLevel((mob, cap, level) -> commandSetLevel(cap, mob, cap.getLevel() + level)))
               )
         );
   }
   ...
   private static Command<CommandSourceStack> mobTrait(LHMobCommands.MobTraitCommand cmd) {
      return ctx -> {
         Reference<MobTrait> trait = resolveKey(ctx, "trait", LHTraits.TRAITS.key(), ERR_INVALID_NAME);
         Collection<? extends Entity> list = EntityArgument.getEntities(ctx, "targets");
         int count = iterate(list, (le, cap) -> cmd.run(le, cap, (MobTrait)trait.value()));
         printCompletion((CommandSourceStack)ctx.getSource(), count);
         return 0;
      };
   }
   ...
   private static Command<CommandSourceStack> mobTraitRank(LHMobCommands.MobTraitRankCommand cmd) {
      return ctx -> {
         ...
         Reference<MobTrait> trait = resolveKey(ctx, "trait", LHTraits.TRAITS.key(), ERR_INVALID_NAME);
         Collection<? extends Entity> list = EntityArgument.getEntities(ctx, "targets");
         int count = iterate(list, (le, cap) -> cmd.run(le, cap, (MobTrait)trait.value(), rank));
         printCompletion((CommandSourceStack)ctx.getSource(), count);
         return 0;
      };
   }
   ...
   private static <T> ResourceKey<T> getRegistryKey(
      CommandContext<CommandSourceStack> ctx, String name, ResourceKey<Registry<T>> reg, DynamicCommandExceptionType err
   ...
   private static <T> Reference<T> resolveKey(
      ...
      return (Reference<T>)getRegistry(ctx, reg).getHolder(ans).orElseThrow(() -> err.create(ans.location()));
   }
