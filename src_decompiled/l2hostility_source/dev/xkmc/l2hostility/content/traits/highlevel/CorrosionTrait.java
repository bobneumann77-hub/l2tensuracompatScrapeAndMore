      ...
      if (!((Abrahadabra)LHItems.ABRAHADABRA.get()).isOn(cache.getTarget())) {
         ...
         if (count < level) {
            cache.addHurtModifier(
               DamageModifier.multTotal(1.0F + (float)((Double)LHConfig.SERVER.corrosionDamage.get() * level * (level - count)), this.getRegistryName())
            );
         }
      }
   }
      ...
      list.add(
         Component.translatable(
               ...
               new Object[]{
                  ...
                  this.mapLevel(
                     access,
                     i -> Component.literal(Math.round((Double)LHConfig.SERVER.corrosionDurability.get() * i.intValue() * 100.0) + "%")
                        .withStyle(ChatFormatting.AQUA)
                  ),
                  this.mapLevel(
                     access,
                     i -> Component.literal(Math.round((Double)LHConfig.SERVER.corrosionDamage.get() * i.intValue() * 100.0) + "%")
                        .withStyle(ChatFormatting.AQUA)
                  )
               }
            )
