...
public class UndyingTrait extends LegendaryTrait {
   ...
   public void onDeath(int level, LivingEntity entity, LivingDeathEvent event) {
      if (!entity.level().isClientSide()) {
         if (!event.getSource().is(DamageTypeTags.BYPASSES_INVULNERABILITY)) {
            if (!((MobTraitCap)((GeneralCapabilityHolder)LHMiscs.MOB.type()).getOrCreate(entity)).hasTrait((MobTrait)LHTraits.SPLIT.get())) {
   ...
   public boolean validTarget(LivingEntity le) {
      return le instanceof EnderDragon ? false : le.canBeAffected(new MobEffectInstance(LCEffects.CURSE, 100));
   }
}
