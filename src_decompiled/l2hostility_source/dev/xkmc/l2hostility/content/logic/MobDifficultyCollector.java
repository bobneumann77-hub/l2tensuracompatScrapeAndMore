...
public class MobDifficultyCollector {
   ...
   public MobDifficultyCollector() {
      this.apply_chance = (Double)LHConfig.SERVER.globalApplyChance.get();
      this.trait_chance = (Double)LHConfig.SERVER.globalTraitChance.get();
      this.suppression = (Double)LHConfig.SERVER.globalTraitSuppression.get();
      this.trait_cost = 1.0;
   }
   ...
   public void acceptConfig(WorldDifficultyConfig.DifficultyConfig config) {
      ...
      this.suppression = 1.0 - (1.0 - this.suppression) * (1.0 - config.suppression());
      this.fullChance = this.fullChance | (this.min > 0 && config.suppression() == 0.0);
   }
   ...
   public void setCap(int cap) {
      if ((Boolean)LHConfig.SERVER.allowBypassMinimum.get()) {
   ...
   public int getDifficulty(RandomSource random) {
      double mean = this.base + this.difficulty * this.scale;
      if (this.count > 0) {
         mean += random.nextGaussian() * Math.sqrt(this.varSq);
      }

      mean *= this.finalFactor;
      return (int)Math.round(Mth.clamp(mean, this.min, this.cap));
   }
   ...
   public double trait_chance(int lv) {
      if (this.delegateTrait) {
         ...
         return this.fullChance ? 1.0 : this.trait_chance * Math.min(1.0, lv * (Double)LHConfig.SERVER.initialTraitChanceSlope.get());
      }
   }

   public double suppression() {
      return this.fullChance ? 1.0 : this.suppression;
   }
   ...
   public boolean hasAdvancement(ResourceLocation id) {
      ...
      AdvancementHolder adv = this.player.server.getAdvancements().get(id);
      if (adv == null) {
         return false;
      }
