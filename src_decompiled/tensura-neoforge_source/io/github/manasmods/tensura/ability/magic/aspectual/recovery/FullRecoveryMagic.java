...
public class FullRecoveryMagic extends AspectualMagic {
   private static final AspectualMagicConfig.FullRecovery CONFIG = ((AspectualMagicConfig)ConfigRegistry.getConfig(AspectualMagicConfig.class)).FullRecovery;

   public FullRecoveryMagic() {
   ...
   public boolean canLearnSkill(ManasSkillInstance instance, LivingEntity entity) {
      ...
      if (!SkillUtils.isSkillMastered(entity, (ManasSkill)AspectualMagics.RECOVERY.get())) {
         ...
         if (entity instanceof Player player) {
            ...
            player.displayClientMessage(
               Component.translatable(
                     ...
                     new Object[]{instance.getChatDisplayName(false), ((RecoveryMagic)AspectualMagics.RECOVERY.get()).getChatDisplayName(false)}
                  )
         ...
         if (target != null) {
            if (target.getHealth() < target.getMaxHealth()) {
               ...
               if (target instanceof Player player) {
                  player.getFoodData().eat(CONFIG.foodPoint, CONFIG.saturationPoint);
               }

               int duration = instance.isMastered(entity) ? CONFIG.absorptionDurationMastered : CONFIG.absorptionDuration;
               MobEffectInstance absorption = new MobEffectInstance(MobEffects.ABSORPTION, duration, CONFIG.absorptionLevel - 1, true, false, true);
               target.addEffect(absorption);
