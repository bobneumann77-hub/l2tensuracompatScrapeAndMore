...
public class IcicleLanceMagic extends AspectualMagic {
   private static final AspectualMagicConfig.IcicleLance CONFIG = ((AspectualMagicConfig)ConfigRegistry.getConfig(AspectualMagicConfig.class)).IcicleLance;

   public IcicleLanceMagic() {
   ...
   public String getModeId(ManasSkillInstance instance, int mode) {
      return mode == 1 ? "icicle_lance.shot" : super.getModeId(instance, mode);
   }
   ...
   public boolean onHeld(ManasSkillInstance instance, LivingEntity entity, int heldTicks, int mode) {
      ...
      if (mode == 0) {
         ...
         if (heldTicks > 0 && heldTicks % CONFIG.castTimeShot == 0) {
            for (int i = 0; i < CONFIG.icicleShotNumber; i++) {
               if (EnergyHelper.isOutOfEnergy(entity, instance, mode)) {
                  return false;
               }

               IceLanceProjectile lance = new IceLanceProjectile(level, entity);
               lance.setSkill(entity, instance, this, mode);
               lance.setSecondaryDamage(CONFIG.magicDamage);
               lance.setSize(0.25F);
               lance.setSpeed(1.75F);
               lance.setNoGravity(true);
               lance.setDelayTick(CONFIG.castTime);
               ...
               lance.setTargetOffset(new Vec3(offsetX, offsetY, 0.25));
               if (lance.getMagicCircle() == null && tag.contains("MagicCircleID")) {
