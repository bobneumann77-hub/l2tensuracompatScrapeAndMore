...
public class ParalysisResistanceSkill extends ResistSkill {
   ...
   public boolean onTakenDamage(ManasSkillInstance instance, LivingEntity owner, DamageSource source, Changeable<Float> amount) {
      if (instance.getMastery() >= 0.0) {
         return super.onTakenDamage(instance, owner, source, amount);
      }

      if (!owner.hasEffect(TensuraMobEffects.getReference(TensuraMobEffects.PARALYSIS))) {
         return true;
      }
   ...
   public List<Holder<MobEffect>> getImmuneEffects(ManasSkillInstance instance, LivingEntity entity) {
      return List.of(TensuraMobEffects.getReference(TensuraMobEffects.PARALYSIS));
   }
