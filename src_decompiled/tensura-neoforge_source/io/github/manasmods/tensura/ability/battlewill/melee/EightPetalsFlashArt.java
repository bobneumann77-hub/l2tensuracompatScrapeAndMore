...
public class EightPetalsFlashArt extends Battlewill {
   private static final BattlewillConfig.EightPetalsFlash CONFIG = ((BattlewillConfig)ConfigRegistry.getConfig(BattlewillConfig.class)).EightPetalsFlash;
   private static final ResourceLocation SLOW = ResourceLocation.fromNamespaceAndPath("tensura", "eight_petals_slash");

   public EightPetalsFlashArt() {
   ...
   public boolean onTakenDamage(ManasSkillInstance instance, LivingEntity entity, DamageSource source, Changeable<Float> amount) {
      ...
      if (!(source.tensura$getBarrierBypassLevel() >= 2.0F) && source.getDirectEntity() != null) {
         Iterator var5 = entity.level()
            .getEntitiesOfClass(HazyBlossomEntity.class, entity.getBoundingBox().inflate(1.0), blossomx -> entity.equals(blossomx.getOwner()))
            .iterator();
         if (var5.hasNext()) {
   ...
   public void onRelease(ManasSkillInstance instance, LivingEntity entity, int heldTicks, int keyNumber, int mode) {
      ...
      if (!(level.getEntity(id) instanceof HazyBlossomEntity blossom)) {
         ...
         float damage = CONFIG.dashDamage;
         double cost = this.getAuraCost(entity, instance, mode);
         if (blossom.isFollowOwner() && blossom.getAge() > 10) {
            if (!instance.isMastered(entity) || blossom.getOwner() != entity) {
               return;
            }

            if (blossom.getPetals() < CONFIG.dashPetal) {
               level.playSound(
                  null,
                  entity.getX(),
                  entity.getY(),
            ...
            damage = blossom.getDamage();
            blossom.setPetals(blossom.getPetals() - CONFIG.dashPetal);
            if (blossom.getPetals() <= 0) {
               blossom.remove();
            }
         } else {
            ...
            if (instance.isMastered(entity)) {
               boostSec = Math.min(CONFIG.bonusChargeTick, heldTicks - CONFIG.chargeTick) / 20;
               cost += boostSec * CONFIG.bonusCost;
               if (EnergyHelper.isOutOfEnergy(entity, cost, 0.0)) {
                  this.removeBlossom(instance, blossom);
                  level.playSound(
                     null,
            ...
            damage += boostSec * CONFIG.bonusDamage;
         }
         ...
         if (level.getBlockState(resultPos).is(TensuraBlockTags.SKILL_NOT_TELEPORTABLE)) {
            ...
            double dashLength = offSetToTarget.length();
            int steps = Mth.floor(dashLength);
            Vec3 dirNorm = offSetToTarget.normalize();

            for (int particleIndex = 1; particleIndex < steps; particleIndex++) {
               Vec3 particlePos = source.add(dirNorm.scale(particleIndex));
               TensuraParticleHelper.addServerParticlesAroundPos(entity.getRandom(), level, particlePos, ParticleTypes.FLASH, 1.0);
            ...
            double inflateRadius = Math.max(entity.getAttributeValue(Attributes.ENTITY_INTERACTION_RANGE) / 2.0, 2.0);
            double radiusSqr = inflateRadius * inflateRadius;
            AABB dashAabb = new AABB(source, vec3).inflate(inflateRadius);
            List<LivingEntity> hits = level.getEntitiesOfClass(LivingEntity.class, dashAabb, targetx -> !targetx.is(entity) && !targetx.isAlliedTo(entity));
            if (!hits.isEmpty()) {
               float baseDamage = (float)entity.getAttributeValue(Attributes.ATTACK_DAMAGE);

               for (LivingEntity target : hits) {
                  if (target.invulnerableTime < 40) {
                     Vec3 tPos = target.position().add(0.0, target.getBbHeight() / 2.0F, 0.0);
                     double t = Mth.clamp(tPos.subtract(source).dot(dirNorm), 0.0, dashLength);
                     Vec3 closest = source.add(dirNorm.scale(t));
                     if (!(tPos.distanceToSqr(closest) > radiusSqr)) {
                        ...
                        if (target.hurt(damageSource, damage + baseDamage)) {
                           ...
                           entity.level()
                              .playSound(null, target.getX(), target.getY(), target.getZ(), SoundEvents.PLAYER_ATTACK_CRIT, entity.getSoundSource(), 1.0F, 1.0F);
                           if (level instanceof ServerLevel serverLevel) {
                              serverLevel.getChunkSource().broadcastAndSend(entity, new ClientboundAnimatePacket(entity, 4));
                           }
                        }

                        TensuraEnchantmentHelper.doAdditionalAfterAttack((ServerLevel)level, target, entity, damageSource, entity.getMainHandItem(), baseDamage);
