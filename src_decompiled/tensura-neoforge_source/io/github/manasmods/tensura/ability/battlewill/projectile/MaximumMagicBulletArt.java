...
public class MaximumMagicBulletArt extends Battlewill {
   private static final BattlewillConfig.MaximumMagicBullet CONFIG = ((BattlewillConfig)ConfigRegistry.getConfig(BattlewillConfig.class)).MaximumMagicBullet;

   @Override
   public boolean checkAcquiringRequirement(Player entity, double newEP) {
      return SkillUtils.isSkillMastered(entity, (ManasSkill)ProjectileArts.MAGIC_BULLET.get());
   }
   ...
   public void onSkillMastered(ManasSkillInstance instance, LivingEntity entity) {
      if (!instance.isSubInstance()) {
         SkillHelper.learnSkill(entity, ((DeathMarchDanceArt)ProjectileArts.DEATH_MARCH_DANCE.get()).createLearningInstance(entity));
      }
   }

   private int getBulletColor(float power) {
      if (power < 5.0F) {
         return AuraBulletProjectile.getColorBySize(power, 0.0F, 5.0F, 10278389, 5416173);
      } else if (power < 10.0F) {
         return AuraBulletProjectile.getColorBySize(power, 5.0F, 10.0F, 5416173, 7747016);
      } else if (power < 15.0F) {
         return AuraBulletProjectile.getColorBySize(power, 10.0F, 15.0F, 7747016, 9109549);
      } else if (power < 20.0F) {
         return AuraBulletProjectile.getColorBySize(power, 15.0F, 20.0F, 9109549, 16733525);
      } else {
         return power < 25.0F
            ? AuraBulletProjectile.getColorBySize(power, 20.0F, 25.0F, 16733525, 16213349)
            : AuraBulletProjectile.getColorBySize(power, 25.0F, 30.0F, 16213349, 16777215);
      }
   }
}
