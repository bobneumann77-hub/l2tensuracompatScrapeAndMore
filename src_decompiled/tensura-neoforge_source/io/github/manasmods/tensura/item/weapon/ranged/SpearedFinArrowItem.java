...
public class SpearedFinArrowItem extends ArrowItem {
   ...
   public AbstractArrow createArrow(Level level, ItemStack itemStack, LivingEntity livingEntity, @Nullable ItemStack itemStack2) {
      return new SpearedFinArrow(level, livingEntity, itemStack.copyWithCount(1), itemStack2);
   }

   public Projectile asProjectile(Level level, Position position, ItemStack itemStack, Direction direction) {
      SpearedFinArrow arrow = new SpearedFinArrow(level, position.x(), position.y(), position.z(), itemStack.copyWithCount(1), null);
