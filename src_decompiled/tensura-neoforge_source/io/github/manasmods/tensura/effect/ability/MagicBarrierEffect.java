...
public class MagicBarrierEffect extends TensuraMobEffect implements DamageAction {
   private static final ResourceLocation MAGIC_BARRIER = ResourceLocation.fromNamespaceAndPath("tensura", "magic_barrier");

   public MagicBarrierEffect() {
      super(MobEffectCategory.BENEFICIAL, new Color(187, 252, 85).getRGB());
      this.addAttributeModifier(TensuraAttributes.MAGIC_BARRIER, MAGIC_BARRIER, 1.0, Operation.ADD_VALUE);
   }
   ...
   public boolean onBeingDamaged(LivingEntity entity, DamageSource source, Changeable<Float> amount) {
      if (!TensuraDamageHelper.isTensuraMagic(source)) {
         ...
         entity.removeEffect(TensuraMobEffects.getReference(TensuraMobEffects.MAGIC_BARRIER));
         ...
         MobEffectInstance barrier = entity.getEffect(TensuraMobEffects.getReference(TensuraMobEffects.MAGIC_BARRIER));
         if (barrier == null) {
            return true;
         } else {
