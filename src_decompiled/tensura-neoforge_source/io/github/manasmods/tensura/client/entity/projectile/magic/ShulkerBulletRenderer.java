...
@OnlyIn(Dist.CLIENT)
public class ShulkerBulletRenderer extends EntityRenderer<TensuraProjectile> {
   ...
   private final ShulkerBulletModel<TensuraProjectile> model;

   public ShulkerBulletRenderer(Context context) {
      super(context);
      this.model = new ShulkerBulletModel(context.bakeLayer(ModelLayers.SHULKER_BULLET));
   }

   protected int getBlockLightLevel(TensuraProjectile shulkerBullet, BlockPos blockPos) {
      return 15;
   }

   public void render(TensuraProjectile shulkerBullet, float f, float g, PoseStack poseStack, MultiBufferSource multiBufferSource, int i) {
      poseStack.pushPose();
      ...
      float k = shulkerBullet.tickCount + g;
   ...
   public ResourceLocation getTextureLocation(TensuraProjectile shulkerBullet) {
