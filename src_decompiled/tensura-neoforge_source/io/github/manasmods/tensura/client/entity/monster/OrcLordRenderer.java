...
public class OrcLordRenderer extends GeoEntityRenderer<OrcEntity> {
         ...
         new BlockAndItemGeoLayer<OrcEntity>(this) {
            ...
            protected ItemStack getStackForBone(GeoBone bone, OrcEntity animatable) {
               return switch (bone.getName()) {
                  case "LeftItem" -> animatable.isLeftHanded() ? OrcLordRenderer.this.mainHandItem : OrcLordRenderer.this.offhandItem;
                  case "RightItem" -> animatable.isLeftHanded() ? OrcLordRenderer.this.offhandItem : OrcLordRenderer.this.mainHandItem;
                  default -> null;
               };
            }
            ...
            protected void renderStackForBone(
               ...
               if (stack == OrcLordRenderer.this.mainHandItem || stack == OrcLordRenderer.this.offhandItem) {
                  poseStack.mulPose(Axis.XP.rotationDegrees(-90.0F));
                  if (stack == OrcLordRenderer.this.mainHandItem && !animatable.isLeftHanded()
                     || stack == OrcLordRenderer.this.offhandItem && animatable.isLeftHanded()) {
                     if (stack.getItem() instanceof ShieldItem || stack.getItem() instanceof SimpleShieldItem) {
                        poseStack.translate(0.0, 0.125, -0.25);
                     }
                  } else if ((
                        stack == OrcLordRenderer.this.mainHandItem && animatable.isLeftHanded()
                           || stack == OrcLordRenderer.this.offhandItem && !animatable.isLeftHanded()
                     )
