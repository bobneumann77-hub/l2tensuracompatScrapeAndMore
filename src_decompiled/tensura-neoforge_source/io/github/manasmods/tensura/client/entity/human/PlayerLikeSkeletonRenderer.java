...
public class PlayerLikeSkeletonRenderer<T extends TamableAnimal> extends LivingEntityRenderer<T, PlayerLikeSkeletonModel<T>> {
   public static final ModelLayerLocation SKELETON = new ModelLayerLocation(ResourceLocation.fromNamespaceAndPath("tensura", "skeleton"), "main");

   public PlayerLikeSkeletonRenderer(Context pContext, float pShadowRadius) {
      super(pContext, new PlayerLikeSkeletonModel(pContext.bakeLayer(SKELETON)), pShadowRadius);
      this.addLayer(
   ...
   private void setModelProperties(T entity) {
      PlayerLikeSkeletonModel<T> model = (PlayerLikeSkeletonModel<T>)this.getModel();
