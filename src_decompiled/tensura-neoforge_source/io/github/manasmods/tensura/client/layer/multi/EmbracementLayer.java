...
public class EmbracementLayer<T extends Entity, M extends EntityModel<T>> extends MultiRenderLayer<T, M> {
   ...
   public EmbracementLayer(RenderLayerParent<T, M> renderLayerParent) {
      super(renderLayerParent);
   }

   public <E extends Entity & GeoEntity> EmbracementLayer(GeoRenderer<E> entityRenderer) {
   ...
   public <E extends Entity & GeoEntity> void renderGeo(
      ...
      if (animatable instanceof LivingEntity entity) {
         if (LustEmbracementEffect.isEmbraced(entity)) {
            ...
            this.getGeoRenderer()
               .reRender(
                  ...
                  Color.ofARGB(200, 77, 75, 0).argbInt()
               );
            poseStack.popPose();
         }
      }
   ...
   public void render(
      ...
      if (entity instanceof LivingEntity living) {
         if (LustEmbracementEffect.isEmbraced(living)) {
            float f = entity.tickCount + partialTicks;
            poseStack.pushPose();
            poseStack.scale(1.0F, 1.0F, 1.0F);
            this.getParentModel()
               .renderToBuffer(
                  poseStack, multiBufferSource.getBuffer(this.getRenderType(f)), packedLight, OverlayTexture.NO_OVERLAY, Color.ofARGB(200, 77, 75, 0).argbInt()
               );
            poseStack.popPose();
         }
      }
