...
public class MetalSlimeRace extends SlimeRace {
   public MetalSlimeRace(Difficulty difficulty) {
      super(difficulty);
   }
   ...
   public MetalSlimeRace() {
      ...
      SlimeConfig.MetalSlime config = ((SlimeConfig)ConfigRegistry.getConfig(SlimeConfig.class)).MetalSlime;
   ...
   public RaceConfig.Default getDefaultConfig() {
      return ((SlimeConfig)ConfigRegistry.getConfig(SlimeConfig.class)).MetalSlime;
   }

   @Override
   protected int getMaxJumpCharge() {
      return ((SlimeConfig)ConfigRegistry.getConfig(SlimeConfig.class)).MetalSlime.maxChargeTick;
   }

   @Override
   public double getCustomWidth() {
      return ((SlimeConfig)ConfigRegistry.getConfig(SlimeConfig.class)).MetalSlime.width;
   }
   ...
   public List<ManasRace> getPreviousEvolutions(ManasRaceInstance instance, LivingEntity entity) {
      return List.of((ManasRace)TensuraRaces.SLIME.get());
   }
   ...
   public Map<EvolutionRequirement, Float> getEvolutionRequirements(ManasRaceInstance previous, LivingEntity entity) {
      return Map.of(
         new EvolutionRequirement.ItemConsumeRequirement(
            (Item)TensuraMaterialItems.MAGIC_ORE.get(), ((SlimeConfig)ConfigRegistry.getConfig(SlimeConfig.class)).MetalSlime.oreRequirement
         ),
         100.0F
      );
   }
   ...
   public List<ManasSkill> getIntrinsicSkills(ManasRaceInstance instance, LivingEntity entity) {
      ...
      list.add((ManasSkill)IntrinsicSkills.BODY_ARMOR.get());
