...
public class EvilCentipedeBody extends LivingMultipartBody implements GeoEntity {
   private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);

   public EvilCentipedeBody(EntityType<? extends EvilCentipedeBody> pEntityType, Level pLevel) {
      super(pEntityType, pLevel);
   }

   public EvilCentipedeBody(EntityType<? extends EvilCentipedeBody> pEntityType, LivingEntity parent) {
   ...
   public boolean isSaddleable() {
      return this.getBodyIndex() == 0 ? false : this.isAlive() && !this.isBaby();
   }
   ...
   public int getChestsAllowed() {
      return this.getBodyIndex() == 0 ? 0 : 1;
   }
   ...
   protected PlayState loopController(AnimationState<EvilCentipedeBody> state) {
      String name;
      if (this.isInLiquid()) {
         name = "animation.evil_centipede.swim";
      } else if (!this.onGround() && !this.isEndSegment()) {
         name = "animation.evil_centipede.fall";
