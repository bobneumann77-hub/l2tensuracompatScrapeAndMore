...
public class SwimmingJumpNavigation extends PathNavigation {
   public SwimmingJumpNavigation(Mob mob, Level worldIn) {
      super(mob, worldIn);
   }

   protected PathFinder createPathFinder(int finder) {
      this.nodeEvaluator = new SwimNodeEvaluator(false);
      return new PathFinder(this.nodeEvaluator, finder);
   }
   ...
   protected void followThePath() {
      if (this.path != null) {
         ...
         if (Math.abs(vector3d1.x) > 0.2 || Math.abs(vector3d1.z) > 0.2) {
            f1 = (float)(f1 * vector3d1.length() * 6.0);
         }

         Vec3 vector3d2 = Vec3.atBottomCenterOf(this.path.getNextNodePos());
         if (Math.abs(this.mob.getX() - vector3d2.x) < f1
            && Math.abs(this.mob.getZ() - vector3d2.z) < f1
            && Math.abs(this.mob.getY() - vector3d2.y) < f1 * 2.0F) {
            this.path.advance();
         }
   ...
   protected void doStuckDetection(Vec3 positionVec3) {
      ...
      if (this.path != null && !this.path.isDone()) {
         ...
         if (vector3i.equals(this.timeoutCachedNode)) {
            ...
            this.timeoutLimit = this.mob.getSpeed() > 0.0F ? d0 / this.mob.getSpeed() * 100.0 : 0.0;
         }
