...
public class SleepMistCloud extends AreaCloud {
   public SleepMistCloud(EntityType<? extends Projectile> pEntityType, Level pLevel) {
      super(pEntityType, pLevel);
   }

   public SleepMistCloud(Level level, LivingEntity entity) {
      this((EntityType<? extends Projectile>)MiscEntityTypes.SLEEP_MIST.get(), level);
   ...
   public void spawnAmbientParticles() {
      ...
      for (int i = 0; i < f; i++) {
         ...
         this.level()
            .addParticle(TensuraParticleUtils.getSleepMist(), this.getX() + pos.x, this.getY() + pos.y, this.getZ() + pos.z, motion.x, motion.y, motion.z);
      }
   }
}
