...
public class FireJailEntity extends BarrierEntity implements GeoEntity {
   private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);

   public FireJailEntity(EntityType<? extends FireJailEntity> entityType, Level level) {
      super(entityType, level);
   }

   public FireJailEntity(Level pLevel, LivingEntity pOwner) {
      this((EntityType<? extends FireJailEntity>)MiscEntityTypes.FIRE_JAIL.get(), pLevel);
   ...
   protected void hitTarget() {
      if (this.getLife() - this.getAge() >= 40) {
         super.hitTarget();
      }
   }
         ...
         new AnimationController[]{
            new AnimationController(
               ...
               event -> this.getLife() - this.getAge() < 60
                  ...
                  : event.setAndContinue(RawAnimation.begin().thenLoop("animation.jail_sphere.loop_fast"))
            ),
            new AnimationController(this, "controller", 0, event -> PlayState.STOP)
               .triggerableAnim("start", RawAnimation.begin().then("animation.jail_sphere.start_fast", LoopType.PLAY_ONCE))
         }
      );
   }
