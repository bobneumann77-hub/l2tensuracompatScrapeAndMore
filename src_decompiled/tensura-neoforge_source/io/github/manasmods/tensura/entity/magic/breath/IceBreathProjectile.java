...
public class IceBreathProjectile extends BreathEntity {
   ...
   public IceBreathProjectile(Level level, LivingEntity entity) {
      this((EntityType<? extends IceBreathProjectile>)MiscEntityTypes.ICE_BREATH.get(), level);
      this.setOwner(entity);
   }

   @Override
   public ResourceKey<DamageType> getDamageType() {
      return TensuraDamageTypes.ICE_BREATH;
   }
   ...
   protected boolean hitEntity(Entity entity, ProjectileHitResult customResult) {
      if (super.hitEntity(entity, customResult)) {
         entity.setTicksFrozen(Math.max(100, 0));
         return true;
      } else {
         return false;
      }
   ...
   public void spawnParticle() {
      if (this.getOwner() instanceof LivingEntity owner) {
         ...
         double z = pos.z;
         double speed = owner.getRandom().nextDouble() * 0.35 + 0.55;
         ...
         for (int i = 0; i < 20; i++) {
            ...
            owner.level().addParticle((ParticleOptions)TensuraParticleTypes.SNOWFLAKE_EFFECT.get(), x + ox, y + oy, z + oz, result.x, result.y, result.z);
         }
      }
   }
}
