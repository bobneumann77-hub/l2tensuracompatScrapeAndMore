...
public class BlackFlameBreathProjectile extends BreathEntity {
   public BlackFlameBreathProjectile(EntityType<? extends BlackFlameBreathProjectile> entityType, Level level) {
      super(entityType, level);
      this.setNoGravity(true);
   }

   public BlackFlameBreathProjectile(Level level, LivingEntity entity) {
      this((EntityType<? extends BlackFlameBreathProjectile>)MiscEntityTypes.BLACK_FLAME_BREATH.get(), level);
   ...
   protected boolean applyMobEffects(LivingEntity target) {
      ...
      MobEffectInstance instance = new MobEffectInstance(
         TensuraMobEffects.getReference(TensuraMobEffects.BLACK_BURN), BlackFlameSkill.CONFIG.blackBurnDuration, 0, true, true, true
      );
      target.setRemainingFireTicks(Math.max(target.getRemainingFireTicks(), BlackFlameSkill.CONFIG.blackBurnDuration));
   ...
   private void applyBlockInteraction(Vec3 pos, BlockPos blockPos, BlockState state) {
      if (state.is(BlockTags.SNOW) || state.is(BlockTags.LEAVES)) {
         ...
         if (BlackFireBlock.canBePlacedAt(this.level(), blockPos.above())
            && !((TensuraSkillEvents.SkillGriefEvent)TensuraSkillEvents.SKILL_GRIEF_PRE.invoker())
               ...
               .isFalse()) {
            this.level().setBlockAndUpdate(blockPos.above(), ((net.minecraft.world.level.block.Block)TensuraBlocks.BLACK_FIRE.get()).defaultBlockState());
            ((TensuraSkillEvents.SkillGriefEvent)TensuraSkillEvents.SKILL_GRIEF_POS.invoker())
               .grief(this.getSkill(), this.level(), this.getOwner(), pos.x(), pos.y() + 1.0, pos.z());
         }
   ...
   public void spawnParticle() {
      if (this.getOwner() instanceof LivingEntity owner) {
         ...
         for (int i = 0; i < 30; i++) {
            ...
            owner.level().addParticle((ParticleOptions)TensuraParticleTypes.BLACK_FIRE.get(), x + ox, y + oy, z + oz, result.x, result.y, result.z);
         }
      }
   }
}
