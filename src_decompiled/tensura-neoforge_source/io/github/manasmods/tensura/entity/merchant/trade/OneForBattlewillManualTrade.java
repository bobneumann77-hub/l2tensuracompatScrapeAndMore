...
public class OneForBattlewillManualTrade implements ItemListing {
   ...
   public OneForBattlewillManualTrade(ItemLike input, TagKey<ManasSkill> output, int maxUses, int xp) {
      this(input, 1, output, maxUses, xp);
   }

   public OneForBattlewillManualTrade(ItemLike input, int inputCount, TagKey<ManasSkill> output, int maxUses, int xp) {
      this(input, inputCount, inputCount, output, maxUses, xp);
   }

   public OneForBattlewillManualTrade(ItemLike input, int minInput, int maxInput, TagKey<ManasSkill> output, int maxUses, int xp) {
      this(input, Pair.of(minInput, maxInput), output, maxUses, xp, 0.05F);
   }

   public OneForBattlewillManualTrade(ItemLike input, Pair<Integer, Integer> inputCount, TagKey<ManasSkill> output, int maxUses, int xp, float priceMultiplier) {
   ...
   public MerchantOffer getOffer(Entity entity, RandomSource randomSource) {
      ...
      ItemStack output;
      if (optional.isEmpty()) {
         output = ((Item)TensuraMaterialItems.BATTLEWILL_MANUAL.get()).getDefaultInstance();
      } else {
         List<Holder<ManasSkill>> random = optional.get().stream().toList();
         output = BattlewillManualItem.createForBattlewill((ManasSkill)random.get(randomSource.nextInt(random.size())).value());
      }
