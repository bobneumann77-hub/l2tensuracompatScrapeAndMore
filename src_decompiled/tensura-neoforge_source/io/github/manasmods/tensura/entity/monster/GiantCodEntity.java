...
public class GiantCodEntity extends TensuraTamableEntity implements GeoEntity, SmartBrainOwner<GiantCodEntity>, ISwimming {
   private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
   private int flopTick = 0;

   public GiantCodEntity(EntityType<? extends GiantCodEntity> type, Level level) {
      super(type, level);
   ...
   public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
      GiantCodEntity fish = (GiantCodEntity)((EntityType)MonsterEntityTypes.GIANT_COD.get()).create(pLevel);
      if (fish == null) {
         return null;
      }
   ...
   public boolean checkSpawnRules(LevelAccessor pLevel, MobSpawnType pSpawnReason) {
      return TensuraEntityTypes.rollSpawn(TensuraEntityTypes.CONFIG.SpawnChance.giantCod, pLevel, pSpawnReason) && super.checkSpawnRules(pLevel, pSpawnReason);
   }
   ...
   public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData) {
      if (pReason == MobSpawnType.NATURAL || pReason == MobSpawnType.CHUNK_GENERATION) {
         for (int i = 0; i <= 3; i++) {
            Cod cod = new Cod(EntityType.COD, this.level());
            cod.setPos(this.getX(), this.getY() - 1.0, this.getZ());
            cod.finalizeSpawn(pLevel, this.level().getCurrentDifficultyAt(cod.blockPosition()), MobSpawnType.NATURAL, null);
            this.level().addFreshEntity(cod);
         }
      }
   ...
   public List<ExtendedSensor<GiantCodEntity>> getSensors() {
      return ObjectArrayList.of(new ExtendedSensor[]{new NearbyLivingEntitySensor(), new HurtBySensor()});
   }

   public BrainActivityGroup<GiantCodEntity> getCoreTasks() {
   ...
   public BrainActivityGroup<GiantCodEntity> getIdleTasks() {
         ...
         new Behavior[]{
            new FirstApplicableBehaviour(
               new ExtendedBehaviour[]{
                  ...
                  TensuraBehaviourHelper.getPreyTargeting(this, Entity::isInWaterOrBubble, entity -> entity.getLastHurtMob() instanceof Cod),
   ...
   public BrainActivityGroup<GiantCodEntity> getFightTasks() {
   ...
   protected PlayState loopController(AnimationState<GiantCodEntity> state) {
